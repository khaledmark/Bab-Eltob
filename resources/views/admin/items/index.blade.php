            ...
            <div class="portlet light bordered">
                <div class="portlet-body">
                    <div class="table-toolbar">
                        <div class="row">
                            <div class="col-lg-6">
                                <a class="btn sbold green" href="{{ route('items.create') }}"> إضافة جديد
                    ...
                    <table class="table table-striped table-bordered table-hover table-checkable order-column" id="sample_1">
                        ...
                        @foreach( $data as $value )
                            <tr class="odd gradeX">
                                ...
                                <td><a href="{{ route('items.show', $value->id) }}"><i class="icon-pencil"></i> {{ $value->name }} </a></td>
                                <td> {{ $value->user->username }} </td>
                                <td> {{ $value->price }} </td>
                                <td> {{ $value->description }} </td>
                                <td> <span class="badge badge-roundless {{ $value->status ? "badge-success" : "label-default" }}"> {{ $value->status ? 'مفعل' :  'موقوف' }}</span></td>
                                <td>
                                    <div class="btn-group">
                                        ...
                                        <ul class="dropdown-menu pull-left" role="menu">
                                            <li>
                                                <a href="{{ route('items.show', $value->id) }}">
                                                    <i class="icon-eye"></i> عرض
                                                </a>
                                            </li>
                                            <li>
                                                <a href="{{ route('items.edit', $value->id) }}">
                                                    <i class="icon-pencil"></i> تعديل
                                                </a>
                                            </li>
                                            <li>
                                                <a class="changeStatus" data="{{ $value->id }}" data_name="{{ $value->name }}" status="{{ $value->status ? '0' : '1' }}">
        ...
        $(document).ready(function() {
            ...
            $('body').on('click', '.delete_data', function() {
                ...
                swal({
                    ...
                    $.ajax({
                        url: "{{ url('/') }}" + "/dashboard/items/destroy" ,
                        type: "POST",
             ...
             $('body').on( "click", '.changeStatus', function( ) {
                ...
                swal({
                    ...
                    $.ajax({
                        url: "{{ url('/') }}" + "/dashboard/items-change-status" ,
                        type: "POST",
