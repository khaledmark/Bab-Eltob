...
@section('page_header')
    <div class="page-bar">
        <ul class="page-breadcrumb">
            ...
            <li>
                <a href="{{ route('cities.index') }}">طلبات السحب </a>
                <i class="fa fa-circle"></i>
            </li>
            <li>
                <span>عرض طلبات السحب  </span>
            </li>
        </ul>
    </div>

    <h1 class="page-title"> عرض طلبات السحب
        <small>عرض طلبات السحب   </small>
    </h1>
@endsection
                    ...
                    <table class="table table-striped table-bordered table-hover table-checkable order-column" id="sample_1">
                        ...
                        @foreach( $data as $value )
                            <tr class="odd gradeX">
                                ...
                                <td> {{ $value->id_num }} </td>
                                <td>
                                    <a href="{{ route('users.show', $value->provider->id) }}"> {{ $value->provider->username }} </a>
                                </td>
                                <td>
                                ...
                                <td> {{ $value->tax }} ر.س</td>
                                <td> {{ $value->totalCost }} ر.س</td>
                                <td> {{  $value->updated_at->format('Y-m-d g:i A') }} </td>
                                <td>
                                    <div class="btn-group">
                                        ...
                                        <ul class="dropdown-menu pull-left" role="menu">
                                            <li>
                                                <a href="{{ route('orders.show', $value->id) }}">
